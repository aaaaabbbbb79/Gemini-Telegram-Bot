...
async def astrology_callback(call: CallbackQuery, bot: TeleBot) -> None:
    ...
    try:
        ...
        if data.startswith("select_sign:"):
            ...
            # 任務交給背景去跑，不卡住 bot 的主循環
            asyncio.create_task(execute_and_save(u_id, bot, call.message, weather_prompts.get(t_type, f"查詢{city}天氣"), virtual_prompt=v_prompt))
            ...
            # 丟到背景執行，防止 Telegram 以為機器人死機而瘋狂重傳
            asyncio.create_task(execute_and_save(u_id, bot, call.message, user_prompt, virtual_prompt=virtual_text))
